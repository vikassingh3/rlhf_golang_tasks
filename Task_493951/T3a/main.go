...
func (task DataMiningTask) Run(wg *sync.WaitGroup, dataIn <-chan interface{}, dataOut chan<- interface{}, errors chan<- error) {
	defer wg.Done()
	...
	if !ok {
		...
		errors <- fmt.Errorf("task failed to receive data")
		return
	}
...
func main() {
	...
	errors := make(chan error, len(tasks)) // Create a channel to collect errors

	for i, task := range tasks {
		dataOut := make(chan interface{}, 1) // Create a new channel for the output of each task

		wg.Add(1)
		go task.Run(&wg, dataIn, dataOut, errors)
